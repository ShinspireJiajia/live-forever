/* ============================================
   綠海養護 - 預約列表頁面互動邏輯
   對應 Figma 節點: 332:572
   參照 crm-handover-service.js 統一互動規範
   ============================================ */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var pendingCards = JSON.parse(localStorage.getItem('crmGreenCareCards') || '[]');
  if (pendingCards.length > 0 && cardList) {
    ...
    // 清除已讀取的暫存資料
    localStorage.removeItem('crmGreenCareCards');
  }
  ...
  if (cardList) {
    cardList.addEventListener('click', function (e) {
      ...
      switch (action) {
        case 'view-record':
          // 檢視紀錄 — 帶上狀態參數導向綠海養護檢視紀錄頁面
          var cardStatus = card ? (card.getAttribute('data-status') || '預約中') : '預約中';
          window.location.href = 'crm-green-care-record.html?status=' + encodeURIComponent(cardStatus);
          break;

        case 'survey':
          // 滿意度問卷
          window.location.href = 'crm-satisfaction-survey.html?no=' + encodeURIComponent(caseId) + '&type=' + encodeURIComponent('綠海養護');
          break;
      }
    });
  }
