...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // 初始化 Canvas 尺寸（使用 offsetWidth/Height 取得旋轉前的佈局尺寸）
  function initCanvas() {
    if (!signatureCanvas || !canvasWrap) return;
    // offsetWidth/offsetHeight 回傳 CSS 佈局尺寸（不受 transform 影響）
  ...
  function getPos(e) {
    ...
    if (isPortrait) {
      // 父容器套用 CSS rotate(90deg)（順時針 90°）
      // 需反轉座標：螢幕座標 → Canvas 內部座標
      // bounding rect 中心 = 元素視覺中心
  ...
  if (btnConfirmSign) {
    btnConfirmSign.addEventListener('click', function () {
      ...
      // 儲存至 localStorage
      try {
        localStorage.setItem('crmSignature', dataUrl);
      } catch (e) {
  ...
  // 頁面載入時恢復已儲存的簽名
  var savedSignature = localStorage.getItem('crmSignature');
