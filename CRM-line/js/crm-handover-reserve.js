...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (btnConfirm && handoverForm) {
    btnConfirm.addEventListener('click', function () {
      ...
      // 清除先前的錯誤提示
      var prevErrors = handoverForm.querySelectorAll('.form-error-msg');
      prevErrors.forEach(function (el) { el.remove(); });
      var prevErrorGroups = handoverForm.querySelectorAll('.has-error');
      ...
      if (hasError) {
        // 滾動到第一個錯誤欄位
        var firstError = handoverForm.querySelector('.has-error');
        if (firstError) {
      ...
      localStorage.setItem('crmHandoverCards', JSON.stringify(pendingCards));
